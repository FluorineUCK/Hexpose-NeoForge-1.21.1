...
object DDAUtils {
	fun raycastBlock(position: Vec3, direction: Vec3, hitTarget: (BlockPos) -> Boolean, exceededBounds: (BlockPos) -> Boolean): List<Iota> {
		val delta = Vec3(abs(1 / direction.x), abs(1 / direction.y), abs(1 / direction.z))
		...
		while (true) {
			...
			if (hitTarget(BlockPos(voxelX, voxelY, voxelZ)))
				return Vec3(voxelX + 0.5, voxelY + 0.5, voxelZ + 0.5).asActionResult
			if (exceededBounds(BlockPos(voxelX, voxelY, voxelZ)))
				return listOf(NullIota())
		}
	}

	fun raycastNormal(position: Vec3, direction: Vec3, hitTarget: (BlockPos) -> Boolean, exceededBounds: (BlockPos) -> Boolean): List<Iota> {
		val delta = Vec3(abs(1 / direction.x), abs(1 / direction.y), abs(1 / direction.z))
		...
		while (true) {
			...
			when (axis) {
				'X' -> {
					voxelX += step
					sideDistanceX += offset
					normal = Vec3(-step.toDouble(), 0.0, 0.0)
				}

				'Y' -> {
					voxelY += step
					sideDistanceY += offset
					normal = Vec3(0.0, -step.toDouble(), 0.0)
				}

				'Z' -> {
					voxelZ += step
					sideDistanceZ += offset
					normal = Vec3(0.0, 0.0, -step.toDouble())
				}
