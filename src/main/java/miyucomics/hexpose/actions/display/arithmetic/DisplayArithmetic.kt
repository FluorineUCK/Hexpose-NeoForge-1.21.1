...
object DisplayArithmetic : Arithmetic {
    ...
    init {
        ...
        implementations[Arithmetic.REPLACE] = object : OperatorBasic(3, IotaMultiPredicate.triple(IotaPredicate.ofType(DisplayIota.TYPE), IotaPredicate.ofType(DoubleIota.TYPE), IotaPredicate.ofType(DisplayIota.TYPE))) {
            override fun apply(iotas: Iterable<Iota>, env: CastingEnvironment): Iterable<Iota> {
                val args = iotas.iterator().withIndex()
        ...
        implementations[Arithmetic.SLICE] = object : OperatorBasic(3, IotaMultiPredicate.triple(IotaPredicate.ofType(DisplayIota.TYPE), IotaPredicate.ofType(DoubleIota.TYPE), IotaPredicate.ofType(DoubleIota.TYPE))) {
            override fun apply(iotas: Iterable<Iota>, env: CastingEnvironment): Iterable<Iota> {
                val args = iotas.iterator().withIndex()
                val display = args.next().value as DisplayIota
                val root = display.getRoot()
    ...
    private fun displayNumberIntoIota(op: (DisplayIota, Double) -> Iota) =
        OperatorBinary(IotaMultiPredicate.pair(IotaPredicate.ofType(DisplayIota.TYPE), IotaPredicate.ofType(DoubleIota.TYPE))) { display, number ->
