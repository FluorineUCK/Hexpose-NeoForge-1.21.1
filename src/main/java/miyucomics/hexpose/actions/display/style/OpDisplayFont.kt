...
object OpDisplayFont : Action {
	val INT_TO_FONT = mapOf(
		0 to ResourceLocation.withDefaultNamespace("default"),
		1 to ResourceLocation.withDefaultNamespace("alt"),
		2 to ResourceLocation.withDefaultNamespace("illageralt")
	)
	...
	override fun operate(env: CastingEnvironment, image: CastingImage, continuation: SpellContinuation): OperationResult {
		...
		if (top is DisplayIota) {
			...
			return OperationResult(image.copy(stack = TreeList.from(stack)).withUsedOp(), listOf(), continuation, HexEvalSounds.NORMAL_EXECUTE.get())
		}
